...
plasmid_range = list(range(start_index[0],end_index[0]+1,1)) # defines indices of where to check for plasmid parts
...
for j in plasmid_range:
    ...
    if j not in plasmid_index:
        ...
        for char in char_to_remove:
             sub_plasmid_str = [item.replace(char,"") for item in sub_plasmid_str]
        sub_plasmid_name = sub_plasmid_str
        print(sub_plasmid_name)
        # print(j) # was used just for troubleshooting
        for name in sub_plasmid_name:
            # only add one instance of the subcomponent
                if name not in sub_p_name: # ex. BioBrick Prefix occurs multiple times, but only want it once
                    sub_comp_plasmid = sbol3.Component(name, sbol3.SBO_DNA)
                    sub_comp_plasmid.name = name
                    sub_p_name[name] = sub_comp_plasmid
                    doc.add(sub_comp_plasmid)
                    plasmid_comp.features.append(sbol3.SubComponent(sub_comp_plasmid))
