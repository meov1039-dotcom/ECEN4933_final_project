...
def purge_unwanted_annotations(promoted_ids, vis_annot_ids):
    ...
    keep_display_ids.update(vis_annot_ids)
...
fallback_keywords = ["pro", "term", "cds", "gene", "rbs"]

def matches_fallback(name):
    name_lower = name.lower()
    return any(kw in name_lower for kw in fallback_keywords)
...
def add_hierarchy(parsed_data):
    ...
    for ann in list(plasmid_def.sequenceAnnotations):
        ...
        if so in visBOL_so or matches_fallback(name) or len(plasmid_def.sequenceAnnotations) == 1:
            print(f"Preserving visible flat annotation: {did} (SO={so}, name={name})")
            vis_annot_ids.append(did)
    ...
    for parent_ann_id, children in nested_map.items():
        ...
        if parent_so in visBOL_so or matches_fallback(parent_name):
        ...
        for child_ann_id in children:
            ...
            parent_loc = parent_ann.locations[0]
            child_loc = child_ann.locations[0]

            parent_start = int(parent_loc.start)
            child_start = int(child_loc.start)
            child_end = int(child_loc.end)

            # ✅ Convert to parent-relative coordinates
            rel_start = child_start - parent_start + 1
            rel_end = child_end - parent_start + 1

            # ✅ Create annotation INSIDE the parent definition
            parent_child_ann = parent_def.sequenceAnnotations.create(child_ann_id + "_inside")
            parent_child_ann.component = child_sub.identity
            parent_child_ann.name = child_name
            parent_child_ann.roles = child_def.roles

            rng = sbol2.Range(start=rel_start, end=rel_end)
            rng.orientation = child_loc.orientation
            parent_child_ann.locations.add(rng)
    ...
    # Final purge:
    purge_unwanted_annotations(promoted_ids, vis_annot_ids)
    ...
    print("Length:")
    print(len(plasmid_def.sequenceAnnotations))
    
    if len(plasmid_def.sequenceAnnotations) < 2:
...
def show_hierarchy_tree(plasmid_def, nested_map, ann_name, original_order):
    ...
    menu.add_command(label="Remove Child Relationship", command=lambda: remove_relationship(tree, nested_map, ann_name, original_order, plasmid_def))
    menu.add_command(label="Promote to Parent", command=lambda: promote_to_parent(tree, nested_map))
    menu.add_command(label="Add Child Here",command=lambda: choose_parent(tree, selected_parent))
    menu.add_command(label="Assign as Child",command=lambda: assign_child(tree, nested_map, selected_parent))
...
# Create function to promote parts:

# Temporary storage for "Add Child Here" workflow
selected_parent = {"id": None}

def promote_to_parent(tree, nested_map):
    """Promote selected node to a parent (root-level)."""
    sel = tree.selection()
    if not sel:
        return
    node_id = sel[0]

    # Ensure it exists as a parent in nested_map
    nested_map.setdefault(node_id, [])

    # Move visually to root
    tree.move(node_id, "", "end")


def choose_parent(tree, selected_parent):
    """Mark selected node as the parent for the next child assignment."""
    sel = tree.selection()
    if not sel:
        return
    selected_parent["id"] = sel[0]


def assign_child(tree, nested_map, selected_parent):
    """Assign selected node as a child of the previously chosen parent."""
    ...
    for p, kids in list(nested_map.items()):
        if child_id in kids:
    ...
    nested_map.setdefault(parent_id, []).append(child_id)

    # Move visually under parent
    tree.move(child_id, parent_id, "end")

    # Reset temporary parent
    selected_parent["id"] = None
...
label1 = Label(root, text="Right-Click on TreeView Objects to Change Hierarchy.").pack(pady=10)
label2 = Label(root, text="Use 'Add Child Here' on the Parent Part of Interest.").pack()
label3 = Label(root, text="Then, Use 'Assign Child' on Child Part of Interest.").pack()
label4 = Label(root, text="'Remove Relationship' Works on Selected Child Parts of Interest").pack()
