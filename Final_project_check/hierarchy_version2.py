...
def add_hierarchy(parsed_data):
    ...
    # Done first to avoid the new parent annotations from getting erased
    # Find all annotation IDs that are parents or children:
    nested_ids = set(nested_map.keys())
    for children in nested_map.values():
        nested_ids.update(children)

    # Filter the remaining "flat" annotations:
    for ann in list(plasmid_def.sequenceAnnotations):
        if ann.displayId in nested_ids:
            continue  # already handled as parent or child

        # Try to map to SO number
        try:
            idx = int(ann.displayId.replace("annotation", ""))
        except ValueError:
            idx = None

        so = so_numbers[idx] if idx is not None and idx < len(so_numbers) else None
        name = ann_name[idx] if idx is not None and idx < len(ann_name) else ann.displayId

        if so in visBOL_so:
            print(f"Flat annotation kept visible: {ann.displayId} (SO={so}, name={name})")
            # leave it in plasmid_def.sequenceAnnotations
        else:
            plasmid_def.sequenceAnnotations.remove(ann.identity)
            print(f"Flat annotation hidden: {ann.displayId} (SO={so}, name={name})")
    ...
    for parent_ann_id, children in nested_map.items():
        ...
        if parent_so in visBOL_so:
            ...
            print(f"Visible parent: {parent_ann.displayId} (SO={parent_so}, name={parent_name})")
        else:
            print(f"Hidden parent: {parent_ann.displayId} (SO={parent_so}, name={parent_name})")
        ...
        # Add children as subcomponents to remain hidden:
    ...
    # Collect all component IDs that are actually used:
    used_defs = set()
    ...
    for ann in plasmid_def.sequenceAnnotations:
        if ann.component:
            used_defs.add(ann.component)

    # Also include children attached under visible parents:
    for comp_def in doc.componentDefinitions:
        for comp in comp_def.components:
            used_defs.add(comp.definition)

    # Remove any unused component definitions:
    for comp_def in list(doc.componentDefinitions):
        if comp_def.identity not in used_defs and comp_def.identity != plasmid_def.identity:
            print(f"Removing ComponentDefinition: {comp_def.displayId}")
            doc.componentDefinitions.remove(comp_def.identity)
    ...
    print("=== Diagnostics ===")
    for comp_def in doc.componentDefinitions:
        print(f"ComponentDefinition: {comp_def.displayId}")
        for comp in comp_def.components:
            print(f"  Subcomponent: {comp.displayId} → {comp.definition}")
        for ann in comp_def.sequenceAnnotations:
            print(f"  Annotation: {ann.displayId} → component={ann.component}")
